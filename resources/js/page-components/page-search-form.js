                ...
                this.vueSearchList = pageSearchlist.getVueApp(
                    Object.assign(option, {
                        afterSearch: function (searchList) {
                            ...
                            // details + summary 表示がある場合、表示を閉じた状態で一覧表示する
                            if ($("details").length > 0) {
            ...
            methods: Object.assign(
                {
                    ...
                    //-----------------------
                    // 検索ボタンクリック Sample Copy
                    //-----------------------
                    btnSearchSampleCopy: function (event) {
                        // チェックボックスにチェックがあるか判定
                        const checkbox = document.getElementsByName("checkBoxes");
                        var isCheck = false;
                        ...
                        if (isCheck) {
                            AjaxCom.getPromise()
                                .then(() => {
                                    // 確認ダイアログ
                                    return appDialogCom.confirmSearchSampleAgain();
                                })
                                .then((flg) => {
                                    if (!flg) {
                                        // いいえを押した場合
