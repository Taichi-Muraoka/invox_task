...
class DbBackup extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $batchMng->batch_type = AppConst::CODE_MASTER_23_2;
            ...
            if (!File::isDirectory($backup_path)) {
                throw new \Exception("Directory does not exist: " . $backup_path);
            }

            // コマンド実行する
            // AWS環境の場合は、mysqldumpコマンドに --set-gtid-purged=OFF オプションを付加する
            // 本番環境がAWS環境ではない場合は、以下の切り分けは不要
            if (config('app.env') === 'production') {
                $command = "mysqldump --host {$host} --single-transaction --no-tablespaces --set-gtid-purged=OFF -u{$user} -p{$pass} {$db_name} -r {$backup_path}{$dump_name} 2>&1";
            } else {
