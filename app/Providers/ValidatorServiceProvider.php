...
class ValidatorServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        Validator::extend('vdTelephone', function ($attribute, $value, $parameters, $validator) {
            return preg_match('/^(?:\+?\d+-)?\d+(?:-\d+){2}$|^\+?\d+$/', $value);
        });

        //-----------------------
        // 郵便番号の形式チェック
        //-----------------------
        // 世界の郵便番号（汎用） 英数字、区切りがある場合はスペースかハイフン
        Validator::extend('vdPostalCode', function ($attribute, $value, $parameters, $validator) {
            return preg_match('/^[A-Z0-9]{2,8}((-|\s)[A-Z0-9]{1,4}){0,2}\z/', $value);
        });
